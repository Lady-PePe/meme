...
/**
 * Ealain\Ealain\Scripts\Component class
 *
 * @package ealain
 */

namespace Ealain\Ealain\Scripts;
...
class Component implements Component_Interface
{
	...
	public function action_enqueue_scripts()
	{
		...
		if (class_exists('ReduxFramework')) {
			if(isset($ealain_options['magic_cursor_btn']) && $ealain_options['magic_cursor_btn'] == 'yes'){
				$mouse_cursor = array(
					'mousecursor' => array(
						'file'   => 'mousecursor.min.js',
						'dependency' => array('jquery'),
						'in_footer' => true,

					),
				);
				$js_files =  array_merge($js_files, $mouse_cursor);
			}
	   }
				
		foreach ($js_files as $handle => $data) {
			$src     = $js_uri . $data['file'];
			$version = ealain()->get_asset_version($js_dir . $data['file']);

			wp_enqueue_script($handle, $src, $data['dependency'], $version, $data['in_footer']);
		}

		wp_localize_script('ealain-loadmore', 'ealain_loadmore_params', array(
		...
		wp_enqueue_script('ealain-loadmore');
	...
	protected function get_js_files(): array
	{
		...
		$js_files = array(
			...
			'gsap'     => array(
				'file'   => 'vendor/gsap.min.js',
				'dependency' => array('jquery'),
				'in_footer' => true,
			),
			'tweenmax'     => array( 
				'file'   => 'vendor/TweenMax.min.js',
				'dependency' => array('jquery'),
				'in_footer' => true,
			),
			'select2'     => array(
