...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>" data-columns="<?php echo esc_attr( $columns ); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;">
	<figure class="woocommerce-product-gallery__wrapper">
        ...
        if($attachment_ids && sizeof($attachment_ids) > 1){
			...
			?>
			<div class="swiper product-single-slider iq-rtl-direction image-slider products ealain-main-product">
				<div class="swiper-wrapper ealain-team ealain-team-slider">
			<?php 
			...
			if ( $attachment_ids ) {
				...
				$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src( 'woocommerce_single' ) ), esc_html__( 'Awaiting product image', 'ealain' ) );
				$html .= '</div>';
				echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', $html, $post_thumbnail_id ); 
