        ...
        <tr
            ...
            <td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>"
                ...
                <a href="<?php echo esc_url( $order->get_view_order_url() ); ?>">
                    <?php echo esc_html( _x( '#', 'hash before order number', 'ealain' ) . $order->get_order_number() ); ?>
                </a>
                ...
                <?php
								...
								echo wp_kses_post( sprintf( _n( '%1$s for %2$s item', '%1$s for %2$s items', $item_count, 'ealain' ), $order->get_formatted_order_total(), $item_count ) );
								?>
                ...
                <?php
								...
								if ( ! empty( $actions ) ) {
									foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
										echo '<a href="' . esc_url( $action['url'] ) . '" class="woocommerce-button ealain-button btn btn-hover ' . sanitize_html_class( $key ) . '"> ' . esc_html( $action['name'] ) . '</a>';
									}
								}
								?>
                <?php endif; ?>
...
<div class="woocommerce-pagination woocommerce-pagination--without-numbers woocommerce-Pagination">
    ...
    <a class="woocommerce-button woocommerce-button--previous woocommerce-Button woocommerce-Button--previous ealain-box-shadow ealain-morden-btn"
        href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page - 1 ) ); ?>">

        <span class="ealain-main-btn">
            <span class="text-btn"><?php esc_html_e( 'Previous', 'ealain' ); ?></span>
        </span>
    </a>
    <?php endif; ?>

    <?php if ( intval( $customer_orders->max_num_pages ) !== $current_page ) : ?>
    <a class="woocommerce-button woocommerce-button--next woocommerce-Button woocommerce-Button--next ealain-box-shadow ealain-morden-btn"
        href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page + 1 ) ); ?>">

        <span class="ealain-main-btn">
            <span class="text-btn"> <?php esc_html_e( 'Next', 'ealain' ); ?></span>
        </span>
    </a>
    <?php endif; ?>
</div>
...
<div
    ...
    <div class="ealain-woocomerce-download-product">
        <div class="ealain-btn-container">
            <a class="ealain-button btn btn-hover woocommerce-Button" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
                <span class="wrap-btn">
                    <span class="text-btn"><?php esc_html_e( 'Browse products', 'ealain' ); ?></span>
                    <span class="btn-img">
        ...
        <p class="ealain-order-msg">
        <?php esc_html_e( 'No order has been made yet.', 'ealain' ); ?>
        </p>
    </div>
