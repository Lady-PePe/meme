...
/**
 * Ealain\Ealain\Accessibility\Component class
 *
 * @package ealain
 */

namespace Ealain\Ealain\Accessibility;
...
class Component implements Component_Interface
{
	...
	public function action_enqueue_navigation_script()
	{
		...
		wp_script_add_data('ealain-navigation', 'async', true);
		wp_script_add_data('ealain-navigation', 'precache', true);
		wp_localize_script(
			'ealain-navigation',
			'ealainScreenReaderText',
			array(
				'expand'   => esc_html__('Expand child menu', 'ealain'),
				'collapse' => esc_html__('Collapse child menu', 'ealain'),
			)
		);
	}
	...
	public function action_print_skip_link_focus_fix()
	{
		...
		wp_register_script('ealain-minified', '', false, true);
		wp_enqueue_script('ealain-minified', '', array(), false, true);
		wp_add_inline_script('ealain-minified', '/(trident|msie)/i.test(navigator.userAgent)&&document.getElementById&&window.addEventListener&&window.addEventListener("hashchange",function(){var t,e=location.hash.substring(1);/^[A-z0-9_-]+$/.test(e)&&(t=document.getElementById(e))&&(/^(?:a|select|input|button|textarea)$/i.test(t.tagName)||(t.tabIndex=-1),t.focus())},!1);');
	}
