...
if ( is_singular( get_post_type() ) ) {
	...
	if ( ! empty( $post->post_parent ) ) {
		...
		if ( ! empty( $prev_link ) ) {
			...
			$attachment_navigation .= '<div class="post-navigation-sub"><span>' . esc_html__( 'Previous:', 'ealain' ) . '</span></div>';
		...
		if ( ! empty( $next_link ) ) {
			...
			$attachment_navigation .= '<div class="post-navigation-sub"><span>' . esc_html__( 'Next:', 'ealain' ) . '</span></div>';
			$attachment_navigation .= $next_link;
			$attachment_navigation .= '</div>';
		}

		if ( ! empty( $attachment_navigation ) ) {
			echo _navigation_markup( $attachment_navigation, $class = 'post-navigation', esc_html__( 'Post navigation', 'ealain' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
