	...
	jQuery(document).ready(function () {
		...
		setTimeout(function () {
			jQuery('.woof_info_popup').on('stylechanged', function () {
				jQuery(this).append('<div class="ealain-show-loader-overlay"></div>');
			});
		}, 500);
...
function ajax_product(all_products, skeleton_view) {
	jQuery.ajax({
		...
		data: {
			loaded_paged: ealain_loadmore_params.current_page
		},
		type: 'POST',
		...
		success: function (res) {
			if (res) {
				...
				all_products.find('.skeleton-main').show();
				loadmore_product();
...
function change_view_btn_event() {
	jQuery('.ealain-view-grid,.ealain-listing').on('click', function () {
		let btn = jQuery(this);
		...
		setCookie('product_view[col_no]', btn.hasClass('ealain-view-grid') ? btn.data('grid') : '1');
		setCookie('product_view[is_grid]', btn.hasClass('ealain-view-grid') ? '2' : '1');

		jQuery('.ealain-product-view-buttons .btn').removeClass('active');
		btn.addClass('active');

		if (btn.hasClass('ealain-listing')) { ////Condition for check switch to list to grid
...
function woof_ajax_done_handler(e) {
	change_view_btn_event();
	loadmore_product(jQuery('.ealain-product-main-list').data('options'));

	ealain_loadmore_params.current_page = 1;
	...
	jQuery.ajax({
		...
		success: function (res) {
			res = JSON.parse(res);
			if (res) {
				ealain_loadmore_params.posts = res['query'];
				ealain_loadmore_params.max_page = res['max_page'];
			}
		}
	});
