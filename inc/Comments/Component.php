...
namespace Ealain\Ealain\Comments;
...
class Component implements Component_Interface, Templating_Component_Interface
{
	...
	public function the_comments(array $args = array())
	{
		...
		$amp_live_list = ealain()->using_amp_live_list_comments();
...
?>
			...
			?>
				<div>
					<button class="button" on="tap:amp-live-comments-list-<?php the_ID(); ?>.update"><?php esc_html_e('New comment(s)', 'ealain'); ?></button>
				</div>
			</amp-live-list>
			<?php
			}
	...
	if (!class_exists('Component_Walker_Comment')) {
		...
		class Component_Walker_Comment extends \Walker_Comment
		{
			...
			?>
					<li <?php comment_class('ealain-comments-item'); ?> id="comment-<?php comment_ID(); ?>">
						<h6 class="mt-0 mb-0"><span class="ealain-type-date"><span class="me-2"><?php echo esc_html__(comment_type() . ':', 'ealain'); ?></span><?php comment_author_link(); ?></span> <?php edit_comment_link(esc_html__('(Edit)', 'ealain'), '<span class="edit-link">', '</span>'); ?>
						</h6>
				...
				?>
					<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
						<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
							<div class="ealain-comments-media">
								<div class="ealain-comment-wrap">
									<div class="ealain-comments-photo">
										<?php if (0 != $args['avatar_size']) echo get_avatar($comment, $args['avatar_size']); ?>
									</div>
									<div class="ealain-comments-info">
										<div class="ealain-comment-metadata">
											<a href="<?php echo esc_url(get_comment_link($comment->comment_ID)); ?>">
												<time datetime="<?php comment_time('c'); ?>">
													<?php printf(wp_kses('%1$s', '1: date'), get_comment_date()); ?>
												</time>
											</a>
											<?php edit_comment_link(esc_html__('Edit', 'ealain'), '<span class="edit-link">', '</span>'); ?>
										</div>
										<!-- .comment-metadata -->
										<h5 class="title">
											<?php printf(wp_kses('%s ', 'ealain'), sprintf('%s', get_comment_author_link())); ?>
										</h5>

										<?php if ('0' == $comment->comment_approved) : ?>
											<p class="comment-awaiting-moderation"><?php esc_html__('Your comment is awaiting moderation.', 'ealain'); ?></p>
										<?php endif; ?>
										<div class="comment-content">
											<?php comment_text(); ?>
										</div><!-- .comment-content -->
										<div class="reply ealain-reply">
											<?php
											$args["reply_text"] = esc_html__('Reply', 'ealain');
